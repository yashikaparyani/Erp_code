...
run_gate "py_compile (api.py)" \
  python -m py_compile gov_erp/api.py

run_gate "py_compile (permission_engine.py)" \
  python -m py_compile gov_erp/permission_engine.py

run_gate "py_compile (hooks.py)" \
  python -m py_compile gov_erp/hooks.py

# ── Gate 2: Structural tests ─────────────────────────────────
run_gate "Structural tests (228+ tests)" \
...
run_gate "Service smoke (hook + permission + import integrity)" \
  python -m pytest gov_erp/tests/test_service_smoke.py \
