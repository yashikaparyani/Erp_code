...
Policy A permission fix (Director tender create/update)
...
Policy A doctype permission alignment
...
Survey API integration with frontend
Files:
...
Unified connector for priority backend APIs
...
APIs successfully connected in this batch
...
APIs where dedicated frontend screen is not found yet
...
Execution page button wiring fixed
...
Inventory page converted from static actions to real workflow actions
Files:
...
Finance Billing now has create + lifecycle action buttons
Files:
...
Finance Costing now has create + lifecycle action buttons
Files:
...
Quick View action for per-record status visibility
Execution support route updates
...
Finance dashboard Generate Invoice button fix
...
O&M Helpdesk Create Ticket button fix (full wiring)
Files:
...
Department Head tender creation permission enabled
Files:
...
OM Operator dashboard access fix (O&M should not require executive/director dashboard)
File:
backend/gov_erp/gov_erp/api.py
