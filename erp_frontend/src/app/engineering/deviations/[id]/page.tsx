...
export default function DeviationDetailPage() {
  ...
  const deviationName = decodeURIComponent((params?.id as string) || '');
  ...
  const loadData = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/engineering/technical-deviations/${encodeURIComponent(deviationName)}`);
      const payload = await res.json();
      if (!payload.success) throw new Error(payload.message || 'Failed to load');
      setData(payload.data?.data || payload.data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load deviation');
    } finally {
      setLoading(false);
    }
  }, [deviationName]);
  ...
  const runAction = async (action: string, extra: Record<string, string> = {}) => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/engineering/technical-deviations/${encodeURIComponent(deviationName)}/actions`, {
        method: 'POST',
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-2 text-gray-500">Loading deviation...</span>
      </div>
    );
  }
  ...
  if (error && !data) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <Link href="/engineering/deviations" className="text-sm text-blue-600 hover:underline">← Back to Deviations</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <Link href="/engineering/deviations" className="mb-3 inline-flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-900">
            <ArrowLeft className="h-3.5 w-3.5" /> Back to Deviations
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">{data.deviation_id || data.name}</h1>
          <p className="mt-1 text-sm text-gray-500">
            {data.linked_project || 'No project'} &middot; {data.linked_site || 'No site'} &middot; {formatDate(data.creation)}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <StatusBadge status={data.status} />
          <ImpactBadge impact={data.impact} />
          {isOpen && canApproveReject && (
            <>
          ...
          {(isApproved || isRejected) && canClose && (
            <button onClick={() => runAction('close')} disabled={!!actionBusy} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 disabled:opacity-50">
              <Lock className="h-3.5 w-3.5" />{actionBusy === 'close' ? 'Closing...' : 'Close'}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="card lg:col-span-1">
          <div className="card-header"><h3 className="font-semibold text-gray-900">Deviation Details</h3></div>
          <div className="card-body">
            <dl className="space-y-3 text-sm">
              {[
                [<Hash key="i" className="h-3.5 w-3.5" />, 'Deviation ID', data.deviation_id || data.name],
                ...
                [<FileText key="d" className="h-3.5 w-3.5" />, 'Drawing', data.linked_drawing],
                [<AlertTriangle key="im" className="h-3.5 w-3.5" />, 'Impact', data.impact],
                [<User key="r" className="h-3.5 w-3.5" />, 'Raised By', data.raised_by],
        ...
        {/* Description + Root Cause + Proposed Solution */}
        <div className="card lg:col-span-2">
          <div className="card-header"><h3 className="font-semibold text-gray-900">Description &amp; Analysis</h3></div>
          <div className="card-body space-y-4">
            ...
            {data.root_cause ? (
              <div>
                ...
                <div className="prose prose-sm max-w-none text-gray-700 whitespace-pre-wrap">{data.root_cause}</div>
              </div>
            ) : null}
            {data.proposed_solution ? (
              <div>
                <h4 className="text-xs font-semibold text-gray-500 uppercase mb-1">Proposed Solution</h4>
                <div className="prose prose-sm max-w-none text-gray-700 whitespace-pre-wrap">{data.proposed_solution}</div>
              </div>
            ) : null}
            {!data.description && !data.root_cause && !data.proposed_solution && (
              <p className="text-sm text-gray-400">No description, root cause, or proposed solution recorded.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {data.linked_project && (
          <Link href={`/engineering/projects/${encodeURIComponent(data.linked_project)}`} className="card card-body flex items-center gap-3 hover:bg-blue-50 transition">
        ...
        {data.linked_drawing && (
          <Link href={`/engineering/drawings/${encodeURIComponent(data.linked_drawing)}`} className="card card-body flex items-center gap-3 hover:bg-purple-50 transition">
            <FileText className="h-5 w-5 text-purple-500" />
            <div>
              <div className="text-xs text-gray-500">Linked Drawing</div>
              <div className="text-sm font-medium text-gray-900">{data.linked_drawing}</div>
            </div>
          </Link>
        )}
      </div>
      ...
      <LinkedRecordsPanel
        links={[
          ...
          },
          {
            ...
            doctype: 'GE Change Request',
            method: 'frappe.client.get_list',
            args: {
              doctype: 'GE Change Request',
              filters: JSON.stringify(data.linked_project ? { project: data.linked_project } : {}),
              fields: JSON.stringify(['name', 'title', 'status', 'project']),
              limit_page_length: '20',
            },
            href: (name) => `/engineering/change-requests/${name}`,
          },
        ]}
      />
      ...
      <div className="card">
        ...
        <div className="card-body">
          <AccountabilityTimeline
            subjectDoctype="GE Technical Deviation"
            subjectName={deviationName}
            compact={false}
            initialLimit={10}
          />
        </div>
      ...
      <ActionModal
        ...
        description={`Reject deviation ${data.deviation_id || data.name}. Please provide a reason.`}
        variant="danger"
        confirmLabel="Reject"
        busy={actionBusy === 'reject'}
        fields={[
          { name: 'reason', label: 'Rejection Reason', type: 'textarea', required: true, placeholder: 'Mandatory — why is this deviation being rejected?' },
        ]}
