...
import {
  ...
  Send, CheckCircle2, XCircle, RefreshCw, IndianRupee, Clock, Hash, Percent,
} from 'lucide-react';
...
export default function EstimateDetailPage() {
  ...
  const estName = decodeURIComponent((params?.id as string) || '');
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/estimates/${encodeURIComponent(estName)}`);
  ...
  const runAction = async (action: string, extra: Record<string, string> = {}) => {
    ...
    try {
      const res = await fetch(`/api/estimates/${encodeURIComponent(estName)}/actions`, {
  ...
  if (loading) return <div className="flex items-center justify-center h-64"><Loader2 className="h-6 w-6 animate-spin text-gray-400" /><span className="ml-2 text-gray-500">Loading estimate...</span></div>;
  if (error && !data) return <div className="flex flex-col items-center justify-center h-64 gap-4"><AlertCircle className="h-10 w-10 text-rose-400" /><p className="text-rose-600">{error}</p><Link href="/finance/estimates" className="text-sm text-blue-600 hover:underline">← Back to Estimates</Link></div>;
  if (!data) return null;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <Link href="/finance/estimates" className="mb-3 inline-flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-900"><ArrowLeft className="h-3.5 w-3.5" /> Back to Estimates</Link>
          <h1 className="text-2xl font-bold text-gray-900">{data.name}</h1>
          <p className="mt-1 text-sm text-gray-500">{data.customer || 'No customer'} &middot; {formatDate(data.estimate_date)}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isApproved && canConvert && (
            <button onClick={() => runAction('convert')} disabled={!!actionBusy} className="inline-flex items-center gap-1.5 rounded-lg bg-purple-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-purple-700 disabled:opacity-50"><RefreshCw className="h-3.5 w-3.5" />{actionBusy === 'convert' ? 'Converting...' : 'Convert to Proforma'}</button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="card lg:col-span-1">
          <div className="card-header"><h3 className="font-semibold text-gray-900">Estimate Details</h3></div>
          <div className="card-body">
            <dl className="space-y-3 text-sm">
              {[
                [<Hash key="n" className="h-3.5 w-3.5" />, 'Estimate', data.name],
                ...
                [<Calendar key="d" className="h-3.5 w-3.5" />, 'Date', formatDate(data.estimate_date)],
                [<Clock key="v" className="h-3.5 w-3.5" />, 'Valid Until', formatDate(data.valid_until)],
      ...
      <LinkedRecordsPanel links={[
        { label: 'Related Proformas', doctype: 'GE Proforma Invoice', method: 'frappe.client.get_list', args: { doctype: 'GE Proforma Invoice', filters: JSON.stringify({ linked_estimate: data.name }), fields: JSON.stringify(['name', 'customer', 'net_amount', 'status']), limit_page_length: '20' }, href: (name) => `/finance/proformas/${name}` },
        { label: 'Related Invoices', doctype: 'GE Invoice', method: 'frappe.client.get_list', args: { doctype: 'GE Invoice', filters: JSON.stringify(data.linked_project ? { linked_project: data.linked_project } : {}), fields: JSON.stringify(['name', 'customer', 'net_receivable', 'status']), limit_page_length: '20' }, href: (name) => `/finance/billing/${name}` },
      ]} />

      <TraceabilityPanel projectId={data.linked_project} />

      <RecordDocumentsPanel referenceDoctype="GE Estimate" referenceName={estName} title="Linked Documents" initialLimit={5} />

      <div className="card"><div className="card-header"><h3 className="font-semibold text-gray-900">Accountability Trail</h3></div><div className="card-body"><AccountabilityTimeline subjectDoctype="GE Estimate" subjectName={estName} compact={false} initialLimit={10} /></div></div>

      <ActionModal open={rejectModal} title="Reject Estimate" description={`Reject estimate ${data.name}. Please provide a reason.`} variant="danger" confirmLabel="Reject" busy={actionBusy === 'reject'} fields={[{ name: 'reason', label: 'Rejection Reason', type: 'textarea', required: true, placeholder: 'Why is this estimate being rejected?' }]} onConfirm={async (values) => { await runAction('reject', { reason: values.reason || '' }); setRejectModal(false); }} onCancel={() => setRejectModal(false)} />
    </div>
  );
}
