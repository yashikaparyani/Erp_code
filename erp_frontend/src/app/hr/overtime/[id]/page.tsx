...
import {
  ArrowLeft, Loader2, AlertCircle, Calendar, User, Clock, Building2,
  Send, CheckCircle2, XCircle, Hash, MapPin, FileText,
} from 'lucide-react';
...
export default function OvertimeDetailPage() {
  ...
  const otName = decodeURIComponent((params?.id as string) || '');
  const { currentUser } = useAuth();

  const [data, setData] = useState<OvertimeDetail | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/overtime/${encodeURIComponent(otName)}`);
      const payload = await res.json();
      if (!payload.success) throw new Error(payload.message || 'Failed to load');
      setData(payload.data?.data || payload.data);
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed to load overtime entry'); }
  ...
  const runAction = async (action: string, extra: Record<string, string> = {}) => {
    ...
    try {
      const res = await fetch(`/api/overtime/${encodeURIComponent(otName)}/actions`, {
  ...
  if (loading) return <div className="flex items-center justify-center h-64"><Loader2 className="h-6 w-6 animate-spin text-gray-400" /><span className="ml-2 text-gray-500">Loading overtime entry...</span></div>;
  if (error && !data) return <div className="flex flex-col items-center justify-center h-64 gap-4"><AlertCircle className="h-10 w-10 text-rose-400" /><p className="text-rose-600">{error}</p><Link href="/hr/overtime" className="text-sm text-blue-600 hover:underline">← Back to Overtime</Link></div>;
  if (!data) return null;

  const st = (data.overtime_status || data.status || 'DRAFT').toUpperCase();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <Link href="/hr/overtime" className="mb-3 inline-flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-900"><ArrowLeft className="h-3.5 w-3.5" /> Back to Overtime</Link>
          <h1 className="text-2xl font-bold text-gray-900">{data.name}</h1>
          <p className="mt-1 text-sm text-gray-500">{data.employee_name || data.employee || 'Unknown'} &middot; {formatDate(data.overtime_date)}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="card-body">
            <dl className="space-y-3 text-sm">
              {[
                ...
                [<Calendar key="d" className="h-3.5 w-3.5" />, 'Date', formatDate(data.overtime_date)],
                [<Clock key="h" className="h-3.5 w-3.5" />, 'Hours', data.overtime_hours != null ? `${data.overtime_hours} hrs` : undefined],
                [<FileText key="r" className="h-3.5 w-3.5" />, 'Rate', data.overtime_rate != null ? `₹${data.overtime_rate}` : undefined],
        ...
        <div className="card lg:col-span-2">
          <div className="card-header"><h3 className="font-semibold text-gray-900">Overtime Summary</h3></div>
          <div className="card-body">
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              <div className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-center"><div className="text-3xl font-bold text-blue-900">{data.overtime_hours ?? 0}</div><div className="text-xs text-blue-600 mt-1">Hours</div></div>
              {data.overtime_rate != null && <div className="rounded-xl border border-amber-100 bg-amber-50 p-4 text-center"><div className="text-2xl font-bold text-amber-900">₹{data.overtime_rate.toLocaleString('en-IN')}</div><div className="text-xs text-amber-600 mt-1">Rate / Hour</div></div>}
              {data.overtime_amount != null && <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-center"><div className="text-2xl font-bold text-emerald-900">₹{data.overtime_amount.toLocaleString('en-IN')}</div><div className="text-xs text-emerald-600 mt-1">Total Amount</div></div>}
            </div>
      ...
      <RecordDocumentsPanel referenceDoctype="GE Overtime Entry" referenceName={otName} title="Linked Documents" initialLimit={5} />

      <div className="card"><div className="card-header"><h3 className="font-semibold text-gray-900">Accountability Trail</h3></div><div className="card-body"><AccountabilityTimeline subjectDoctype="GE Overtime Entry" subjectName={otName} compact={false} initialLimit={10} /></div></div>

      <ActionModal open={rejectModal} title="Reject Overtime Entry" description={`Reject overtime entry ${data.name}. Please provide a reason.`} variant="danger" confirmLabel="Reject" busy={actionBusy === 'reject'} fields={[{ name: 'reason', label: 'Rejection Reason', type: 'textarea', required: true, placeholder: 'Why is this overtime entry being rejected?' }]} onConfirm={async (values) => { await runAction('reject', { reason: values.reason || '' }); setRejectModal(false); }} onCancel={() => setRejectModal(false)} />
    </div>
  );
}
