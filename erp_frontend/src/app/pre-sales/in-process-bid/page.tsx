...
export default function InProcessBidPage() {
  const [rows, setRows] = useState<InProcessBidRow[]>([]);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-[var(--text-soft)] mt-0.5">
            Accepted won bids with tenure tracking and closure readiness
          </p>
        </div>
        <button
                ...
                <tr key={row.bid_id} className="hover:bg-[var(--surface-hover)] group">
                  ...
                  <td className="px-3 py-3">
                    {typeof row.days_left === 'number' ? (
                      <span className={row.days_left <= 90 ? 'font-semibold text-amber-700' : 'text-[var(--text-main)]'}>
                        {row.days_left} days
                      </span>
                    ) : (
