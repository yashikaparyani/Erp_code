...
const ACTION_METHODS: Record<string, string> = {
  submit: 'submit_attendance_regularization',
  approve: 'approve_attendance_regularization',
  reject: 'reject_attendance_regularization',
  reopen: 'reopen_attendance_regularization',
};
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!method) {
      return NextResponse.json({ success: false, message: 'Unsupported regularization action' }, { status: 400 });
    }
    ...
    return jsonErrorResponse(error, 'Failed to run regularization action');
  }
}
