...
const ACTION_TO_METHOD: Record<string, string> = {
  submit: 'submit_cost_sheet_for_approval',
  approve: 'approve_cost_sheet',
  reject: 'reject_cost_sheet',
  revise: 'revise_cost_sheet',
};
...
export async function POST(
  ...
  try {
    ...
    const costSheetName = decodeURIComponent(id);
    ...
    const args: Record<string, any> = { name: costSheetName };
    ...
    return NextResponse.json(
      { success: false, message: error instanceof Error ? error.message : 'Cost sheet action failed' },
      { status: 500 },
    );
  }
}
